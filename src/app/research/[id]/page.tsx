        ...
        <div className="flex flex-col md:flex-row gap-6">
          ...
          <div className="w-full md:w-1/3">
            <div className="bg-gray-50 p-4 rounded-lg mb-6">
              <h2 className="text-lg font-semibold mb-4">Members ({group.memberCount})</h2>
                ...
                <div className="mb-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {group.adminInfo.map((admin) => (
                      <div key={`admin-${admin.id}`} className="flex flex-col items-center">
                        <Avatar className="h-10 w-10">
                          <AvatarImage src={admin.imageUrl} alt={admin.name} />
                          <AvatarFallback>{admin.name.charAt(0)}</AvatarFallback>
                        </Avatar>
                        <span className="text-xs mt-1 truncate max-w-[50px]">{admin.name.split(' ')[0]}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="mb-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {group.memberInfo.map((member) => (
                      <div key={`member-${member.id}`} className="flex flex-col items-center">
                        <Avatar className="h-10 w-10">
                          <AvatarImage src={member.imageUrl} alt={member.name} />
                          <AvatarFallback>{member.name.charAt(0)}</AvatarFallback>
                        </Avatar>
                        <span className="text-xs mt-1 truncate max-w-[50px]">{member.name.split(' ')[0]}</span>
                      </div>
                    ))}
                  </div>
                </div>
